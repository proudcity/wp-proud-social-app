...
// Main Application
//***************************************
angular.module("socialAppParent",["socialApp","angular-inview","angular-lazycompile","readMore"]),angular.module("socialApp",[
// 'iso.directives',
"ngResource","ngSanitize","dynamicLayout"]).run(["$rootScope","$window",function($rootScope,$window){$rootScope.socialApi=_.get(Proud,"settings.proud_social_app.socialApi")||"https://feeds.proudcity.com:8443/api/";var user=_.get(Proud,"settings.global.location.city")||"";user&&(user+=", "+_.get(Proud,"settings.global.location.state")||"",user=user.replace(/ /g,"_")),$rootScope.socialUser=user||"West_Carrollton,_Ohio"}]).factory("SocialFeed",["$resource","$rootScope",function($resource,$rootScope){var baseUrl=$rootScope.socialApi+"/"+$rootScope.socialUser+"/feed";return{getFeed:function(){return $resource(baseUrl,{format:"json",action:"query",callback:"JSON_CALLBACK"},{query:{cache:!0,method:"GET",isArray:!0}})}}}]).filter("parseSocialText",function(){var urlPattern=/(http|ftp|https):\/\/[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?/gi,hashPattern=/(^|\s)#(\w*[a-zA-Z_]+\w*)/gim,mentionPattern=/(^|\s)\@(\w*[a-zA-Z_]+\w*)/gim;return function(text,target,service){var replacedText=text?text.replace(urlPattern,'<a target="'+target+'" href="$&">$&</a>'):text;
...
self.calculateAspectRatios=function(social,callback){var imageCount={};_.map(social,function(item,key){function imagesChecker(){delete imageCount[key],
// Finished with images
_.size(imageCount)||callback(social)}if(
// Add template file
social[key].template="views/apps/socialApp/default-card-style.html",item.image){
// Create new offscreen image to test
var theImage=new Image;theImage.src=item.image,imageCount[key]=!0,theImage.onerror=function(){imagesChecker()},
// Get accurate measurements from that.
theImage.addEventListener("load",function(){this.naturalWidth>=this.naturalHeight?social[key].styleAttr=this.naturalHeight/this.naturalWidth*100:(social[key].styleAttr=100,social[key].imgStyleAttr=(this.naturalHeight/this.naturalWidth*100-100)/3),imagesChecker()},!0)}})},
...
$scope.inited||self.applyActiveServices(data),data=_.slice(self.preSort?_.chain(data).sortBy("date").reverse().value():data,0,limit),self.calculateAspectRatios(data,function(){$scope.inited=!0,callback(data)})})},
// Toggle social source
$scope.switchService=function(event,service,limit,callback){callback=callback||function(){},event&&event.preventDefault();var isActive=$scope.isServiceActive(service),runQuery=!1,callback=callback||function(){},limit=limit||$scope.socialPostCount;
// switch active
...
$scope.$evalAsync(function($scope){$scope.social=data})})},
// We pre-sorting?
...
// call init
$scope.inited||$scope.switchService(null,null,$scope.socialPostCount),$scope.toggleText=function(){$timeout(function(){$rootScope.$broadcast("dynamicLayout.layout")})},
// Watch social
$scope.$watch("social",function(value){$scope.social&&console.log("ok")})}}}).directive("socialStatic",function($window,$browser,$http,$timeout){return{restrict:"A",controller:"SocialController",templateUrl:"views/apps/socialApp/social-static.html",link:function($scope,$element,$attributes){
// Init vars
