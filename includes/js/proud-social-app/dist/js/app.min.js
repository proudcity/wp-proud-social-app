...
// 'iso.directives',
"ngResource","ngSanitize","dynamicLayout","readMore"]).run(["$rootScope","$window",function($rootScope,$window){$rootScope.socialApi=_.get(Proud,"settings.proud_social_app.socialApi")||"https://feeds.proudcity.com:8443/api/";var user=_.get(Proud,"settings.global.location.city")||"";user&&(user+=", "+_.get(Proud,"settings.global.location.state")||"",user=user.replace(/ /g,"_")),$rootScope.socialUser=user||"West_Carrollton,_Ohio"}]).factory("SocialFeed",["$resource","$rootScope",function($resource,$rootScope){var baseUrl=$rootScope.socialApi+"/"+$rootScope.socialUser+"/feed";return{getFeed:function(){return $resource(baseUrl,{format:"json",action:"query",callback:"JSON_CALLBACK"},{query:{cache:!0,method:"GET",isArray:!0}})},getCustomFeed:function(accounts){return $resource(baseUrl+"/accounts/"+accounts,{format:"json",action:"query",callback:"JSON_CALLBACK"},{query:{cache:!0,method:"GET",isArray:!0}})}}}]).filter("parseSocialText",function(){var urlPattern=/(http|ftp|https):\/\/[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?/gi,hashPattern=/(^|\s)#(\w*[a-zA-Z_]+\w*)/gim,mentionPattern=/(^|\s)\@(\w*[a-zA-Z_]+\w*)/gim;return function(text,target,service){var replacedText=text?text.replace(urlPattern,'<a target="'+target+'" href="$&">$&</a>'):text;
